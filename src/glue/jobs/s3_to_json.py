        ...
        for json_line in p:
            ...
            if dataset_identifier == "EnrolledParticipants":
                for field_name in ["Symptoms", "Treatments"]:
                    if field_name in j["CustomFields"] and isinstance(
                        j["CustomFields"][field_name], str
                    ):
...
def process_record(s3_obj: dict, workflow_run_properties: dict) -> None:
    """
    ...
    with zipfile.ZipFile(io.BytesIO(s3_obj["Body"])) as z:
        ...
        for json_path in non_empty_contents:
            ...
            logger.info("Writing %s to dataset %s", json_path, dataset_identifier)
...
def main() -> None:
    ...
    for message in messages:
        logger.info(
            "Retrieving S3 object for Bucket %s and Key %s",
        ...
        process_record(s3_obj=s3_obj, workflow_run_properties=workflow_run_properties)
